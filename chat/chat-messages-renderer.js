...
function renderSingleMessage(docId, text, sender, isMe, timeStr, isGroup, isDeleted, isEdited, senderAvatar, isSenderVerified) {
    ...
    // Allineamento a destra (tu) o sinistra (altri) con supporto gap elastico per avatar
    ...
    // Avatar dell'utente posizionato a sinistra per i messaggi ricevuti
    let avatarHtml = '';
    if (!isMe) {
        // Se è un messaggio di sistema, forziamo il background grigio passandolo come classe custom
    ...
    // MENU CONTESTUALE (POPOVER MODIFICA/ELIMINA)
    let actionMenu = '';
    if (isMe && !isDeleted) {
