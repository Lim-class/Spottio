...
function renderSingleMessage(docId, text, sender, isMe, timeStr, isGroup, isDeleted, isEdited, senderAvatar, isSenderVerified) {
    const messagesContainer = document.getElementById('messages-container');
    if (!messagesContainer) return;

    const msgDiv = document.createElement('div');
    msgDiv.className = `w-full flex ${isMe ? 'justify-end' : 'justify-start'} mb-3 px-2 items-start gap-2.5`;
    ...
    const verifiedBadge = window.Spottio.getVerifiedBadge(isSenderVerified, "w-3.5 h-3.5 text-blue-500 inline-block align-middle ml-0.5");

    const senderLabel = (!isMe && isGroup && sender !== "Sistema") 
        ? `<span class="text-[10px] font-bold text-green-600 block mb-1">${sender}${verifiedBadge}</span>` 
        : '';

    let avatarHtml = '';
    if (!isMe) {
        const customClasses = sender === 'Sistema' ? "w-8 h-8 mt-0.5 text-xs !bg-gray-400 !bg-none" : "w-8 h-8 mt-0.5 text-xs";
        avatarHtml = window.Spottio.getAvatarHtml(senderAvatar, sender, customClasses);
    }
    ...
    if (isDeleted) {
        ...
        if (text.startsWith('image:') || text.startsWith('video:') || text.startsWith('audio:')) {
            ...
            if (type === 'image') {
                messageContent = `<a href="${url}" target="_blank"><img src="${url}" class="rounded-xl max-h-60 object-cover cursor-pointer hover:opacity-95 transition-opacity mt-1" alt="Immagine allegata"></a>`;
            } else if (type === 'video') {
                messageContent = `<video src="${url}" controls class="rounded-xl max-h-60 w-full focus:outline-none mt-1"></video>`;
            } else if (type === 'audio') {
                messageContent = `<audio src="${url}" controls class="w-full max-w-xs mt-1"></audio>`;
            }
        } else {
            let safeFormattedText = text.replace(/</g, "&lt;").replace(/>/g, "&gt;");
            
            // Evidenzia le menzioni se ci troviamo in un gruppo
            if (isGroup) {
                safeFormattedText = safeFormattedText.replace(/@(\w+)/g, '<span class="text-blue-600 font-bold bg-blue-50 px-1 rounded shadow-sm">@$1</span>');
            }

            messageContent = `<p class="text-sm leading-relaxed whitespace-pre-wrap" style="overflow-wrap: anywhere; word-break: break-word;">${safeFormattedText}</p>`;
        }
    }
