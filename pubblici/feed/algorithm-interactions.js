...
window.toggleLike = function(postId) {
    ...
    postRef.get().then((doc) => {
        ...
        if (hasLiked) {
            ...
            // Riduciamo il punteggio tramite l'algoritmo centralizzato
            if (window.FeedAlgorithm) {
                window.FeedAlgorithm.updateScore(postCategory, -1);
            }
            ...
            // Aumentiamo il punteggio tramite l'algoritmo centralizzato
            if (window.FeedAlgorithm) {
                window.FeedAlgorithm.updateScore(postCategory, 1);
            }
