document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initPrivacySettings() {
        ...
        // Recupera l'UID univoco
        const currentUid = window.Spottio ? window.Spottio.getCurrentUid() : (window.auth?.currentUser?.uid || localStorage.getItem('currentUid'));

        if (!currentUid) return;

        try {
            // Cerca il documento usando l'UID
        ...
        togglePrivacy.addEventListener('change', async (e) => {
            ...
            try {
                // Aggiorna o crea il documento usando l'UID
                await window.db.collection('users').doc(currentUid).set({
                    isPrivate: isPrivate
                }, { merge: true });
                ...
                statusMsg.textContent = isPrivate ? t.accountPrivateStatus : t.accountPublicStatus;
    ...
    function initDataExport() {
        ...
        async function fetchUserData() {
            const currentUid = window.Spottio ? window.Spottio.getCurrentUid() : (window.auth?.currentUser?.uid || localStorage.getItem('currentUid'));
            const currentUser = localStorage.getItem('currentUser') || "Utente";

            if (!currentUid) return {};

            // Recupera i dati del profilo tramite UID
            const userDoc = await window.db.collection('users').doc(currentUid).get();
            const userData = userDoc.exists ? userDoc.data() : {};

            // --- GESTIONE DATI ALGORITMO ---
            ...
            // --- RISOLTO: Cerca i post usando il campo 'user' (dove è memorizzato l'UID) ---
            let postsQuery = await window.db.collection('posts')
                .where('user', '==', currentUid) 
                .get();

            // Fallback: se la collezione è davvero vecchia e usa il nome utente testuale
            if (postsQuery.empty) {
                postsQuery = await window.db.collection('posts')
                    .where('author', '==', currentUser) // Nota: ho usato 'author' come nel database screen
                    .get();
                
                // Fallback finale per le versioni intermedie
                if (postsQuery.empty) {
                     postsQuery = await window.db.collection('posts')
        ...
        btnExportJson.addEventListener('click', async () => {
            ...
            btnExportJson.innerHTML = t.processingBtn;
        ...
        btnExportHtml.addEventListener('click', async () => {
            ...
            btnExportHtml.innerHTML = t.processingBtn;
