...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadNavigation() {
        ...
        if (navContainer) {
            // Sostituisci la sezione <nav> all'interno di nav.js con questa:
const navHTML = `
<nav class="flex flex-col space-y-3 w-full">
    <a href="../spot/spot.html" class="nav-link text-lg font-medium text-gray-600 hover:bg-gray-100 p-2.5 rounded-xl transition duration-200 flex items-center gap-3" data-translate="navPubblici">
    ...
    <a href="../spotini/spotini.html" class="nav-link text-lg font-medium text-gray-600 hover:bg-gray-100 p-2.5 rounded-xl transition duration-200 flex items-center gap-3" data-translate="navSpotini">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
        <span class="nav-text">Spotini</span>
    </a>
    <a href="../cerca-persone/cerca-persone.html" class="nav-link text-lg font-medium text-gray-600 hover:bg-gray-100 p-2.5 rounded-xl transition duration-200 flex items-center gap-3" data-translate="navCercaPersone">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
        <span class="nav-text">Cerca</span>
    </a>
    <a href="../posta/posta.html" class="nav-link text-lg font-medium text-gray-600 hover:bg-gray-100 p-2.5 rounded-xl transition duration-200 flex items-center gap-3" data-translate="navPosta">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
        </svg>
        <span class="nav-text">Posta</span>
    </a>
    <a href="../chat/chat.html" class="nav-link text-lg font-medium text-gray-600 hover:bg-gray-100 p-2.5 rounded-xl transition duration-200 flex items-center gap-3" data-translate="navMessaggi">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
        </svg>
        <span class="nav-text">Messaggi</span>
    </a>
    <a href="../profilo/profilo.html" class="nav-link text-lg font-medium text-gray-600 hover:bg-gray-100 p-2.5 rounded-xl transition duration-200 flex items-center gap-3" data-translate="navProfilo">
        <img id="nav-profile-img" 
             src="https://i.ibb.co/b5HgvzCB/Spottio-Logo-2.png" 
             class="w-6 h-6 rounded-full object-cover shrink-0 border border-gray-200 shadow-sm" 
             alt="Profilo">
...
// --- PROTEZIONE REAL-TIME UTENTI BANNATI ---
(function initRealtimeBanGuard() {
    const auth = window.auth || (typeof firebase !== 'undefined' ? firebase.auth() : null);
    const db = window.db || (typeof firebase !== 'undefined' ? firebase.firestore() : null);
    ...
    auth.onAuthStateChanged((user) => {
        ...
        // Ascolto in tempo reale delle modifiche al profilo utente
        db.collection("users").doc(user.uid).onSnapshot(async (docSnapshot) => {
            ...
            if (banned && banned.status === true) {
                if (banned.bannedUntil) {
                    const now = new Date();
                    const expiry = banned.bannedUntil.toDate ? banned.bannedUntil.toDate() : new Date(banned.bannedUntil);
                    if (now < expiry) {
                        isBanned = true;
                        reason = banned.reason || "Violazione del regolamento";
                    }
                } else {
                    isBanned = true;
                    reason = banned.reason || "Violazione del regolamento";
                ...
                isBanned = true;
                reason = "Sospensione da parte dell'amministratore";
            }

            // Se l'utente risulta bannato mentre è attivo nell'app:
            if (isBanned) {
                console.warn("Account bannato durante la sessione attiva. Espulsione...");
                await auth.signOut();
                ...
                // Reindirizza a login mostrando la motivazione
                window.location.href = `../login/login.html?banned=true&reason=${encodeURIComponent(reason)}`;
            }
        }, (error) => {
            console.error("Errore listener real-time ban:", error);
        });
    });
})();