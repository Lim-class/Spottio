document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // CARICAMENTO INIZIALE DELLA LINGUA
    // ==========================================
    
    // ==========================================
    // CARICAMENTO INIZIALE DELLA LINGUA (FIX TEMPISMO)
    // ==========================================
    
    // Aspettiamo 100 millisecondi per assicurarci che nav.js e il footer abbiano generato l'HTML
    setTimeout(() => {
        // Recupera la lingua salvata, altrimenti imposta l'italiano ('it') di default
        const savedLang = localStorage.getItem('selectedLanguage') || 'it';
        
        // Trova l'opzione salvata
        const initialOption = document.querySelector(`.lang-option[data-lang="${savedLang}"]`);
        
        // Ripesca gli elementi del pulsante (potrebbero essere stati generati in ritardo)
        const currentSelectedFlag = document.getElementById('selected-flag');
        const currentSelectedLangText = document.getElementById('selected-lang-text');

        // Aggiorna visivamente il pulsante con la bandiera corretta
        if (initialOption && currentSelectedFlag && currentSelectedLangText) {
            currentSelectedFlag.src = initialOption.querySelector('img').src;
            currentSelectedFlag.alt = `Bandiera ${initialOption.getAttribute('data-text')}`;
            currentSelectedLangText.textContent = initialOption.getAttribute('data-text');
        }

        // Esegui la traduzione su tutto il documento ora che è pronto
