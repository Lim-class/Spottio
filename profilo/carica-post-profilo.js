async function loadUserPosts() {
    ...
    // Helper interno per facilitare la traduzione (con fallback al testo originale)
    ...
    let usernameToSearch = localStorage.getItem('currentUser');

    if (!usernameToSearch && (window.auth || typeof firebase !== 'undefined')) {
        const authInstance = window.auth || firebase.auth();
        if (authInstance.currentUser) {
            usernameToSearch = authInstance.currentUser.displayName || authInstance.currentUser.email.split('@')[0];
        }
    }
    ...
    console.log("🔍 DEBUG - Sto cercando i post pubblicati da:", usernameToSearch);

    try {
        const postsRef = dbInstance.collection('posts')
                                   .where('user', '==', usernameToSearch)
                                   .orderBy('timestamp', 'desc');

        const snapshot = await postsRef.get();
        
        console.log("🔍 DEBUG - Post trovati per questo utente:", snapshot.size);

        // Aggiorniamo il numero dei post targetizzando lo span specifico (il testo "Post" verrà tradotto automaticamente via data-translate nell'HTML)
        ...
        if (snapshot.empty) {
            container.innerHTML = `<p class="text-gray-400 italic col-span-full text-center py-4">${tx('noPostsFound', "Nessun post trovato per l'utente")} <b>${usernameToSearch}</b>.</p>`;
            return;
        }
        ...
        snapshot.forEach(doc => {
            ...
            let dateString = '';
            if (post.timestamp) {
             // Usa .toDate() se è un Timestamp di Firestore, altrimenti usa new Date come fallback di sicurezza
        ...
        if (error.message && error.message.includes("requires an index")) {
            container.innerHTML = `<p class="text-orange-600 col-span-full text-center text-sm p-4 bg-orange-50 rounded-xl border border-orange-200">
                ...
                Firebase richiede un nuovo indice per cercare per <b>user</b> e ordinare per <b>timestamp</b>.<br>
