...
document.addEventListener('DOMContentLoaded', function () {
  ...
  localStorage.setItem('currentUsername', 'utenteLoggato'); // --- FINE FIX TEMPORANEO ---

  var username = localStorage.getItem('currentUserProfile');
  var profileName = document.getElementById('user-profile-name');
  var pageTitle = document.getElementById('user-profile-title');
  var profileImageContainer = document.getElementById('profile-image-container');
  var profileBio = document.getElementById('user-profile-bio');
  ...
  var followingCount = document.getElementById('following-count'); // Funzione per aggiornare il pulsante "Segui"
  ...
  function toggleFollow() {
    ...
    var followingData = JSON.parse(localStorage.getItem('following')) || {};
    var followersData = JSON.parse(localStorage.getItem('followers')) || {}; // Controlla se l'utente sta già seguendo il profilo

    var isFollowing = followingData[loggedInUser] && followingData[loggedInUser].includes(username);

    if (isFollowing) {
      // Rimuovi l'utente seguito
      followingData[loggedInUser] = followingData[loggedInUser].filter(function (user) {
        return user !== username;
      }); // Rimuovi il follower

      if (followersData[username]) {
        followersData[username] = followersData[username].filter(function (user) {
          return user !== loggedInUser;
        });
      }

      console.log("Hai smesso di seguire ".concat(username, "."));
    } else {
      ...
      followingData[loggedInUser].push(username); // Aggiungi il follower
      ...
      console.log("Ora segui ".concat(username, "."));
  ...
  function updateFollowCounts() {
    var loggedInUser = localStorage.getItem('currentUsername');
    var followersData = JSON.parse(localStorage.getItem('followers')) || {};
    var followingData = JSON.parse(localStorage.getItem('following')) || {}; // Conteggio dei follower del profilo visualizzato

    var currentProfileFollowers = followersData[username] ? followersData[username].length : 0;
    followersCount.textContent = currentProfileFollowers; // Il conteggio dei seguiti deve essere quello dell'utente loggato, non del profilo visualizzato

    var loggedInUserFollowing = followingData[loggedInUser] ? followingData[loggedInUser].length : 0; // Se il profilo visualizzato è l'utente loggato, aggiorna anche il conteggio dei seguiti
  ...
  if (username) {
    ...
    pageTitle.textContent = "Profilo di " + username; // Controlla se l'utente del profilo è l'utente corrente
