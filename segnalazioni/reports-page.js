...
async function loadReports() {
    ...
    if (currentUid && currentUid !== "null") {
        try {
            // Interroga direttamente il Database per il vero stato Admin
            const userDoc = await window.db.collection("users").doc(currentUid).get();
            if (userDoc.exists) {
                dbIsAdmin = userDoc.data().isAdmin === true;
            }
        } catch (error) {
            console.error("Errore verifica privilegi in reports:", error);
        }
    }

    // Se il DB dice che NON sei admin, blocca l'accesso alla pagina
    if (!dbIsAdmin) {
        reportsContainer.innerHTML = '<p class="text-red-500 text-center text-lg font-bold mt-10">Accesso negato. Solo gli amministratori possono vedere questa pagina.</p>';
