...
const APP_LANGUAGES = [
    ...
    { code: 'ko-kp', text: '조선말 (Nord)', flag: 'kp.svg', alt: 'Bandiera Corea del Nord' },
    { code: 'ko-kr', text: '한국어 (Sud)', flag: 'kr.svg', alt: 'Bandiera Corea del Sud' }
];

window.APP_LANGUAGES = APP_LANGUAGES;

/**
 * Normalizza il codice lingua verso la chiave corretta del dizionario.
 * Selezionando 'ko-kp' o 'ko-kr', restituisce 'ko'.
 */
function getDictionaryKey(langCode) {
    if (!langCode) return 'it';
    const clean = String(langCode).toLowerCase().trim();
    if (clean === 'ko-kp' || clean === 'ko-kr' || clean.startsWith('ko')) {
        return 'ko';
    }
    return clean;
}

window.getDictionaryKey = getDictionaryKey;
...
function getLanguageDropdownHTML() {
    const optionsHtml = APP_LANGUAGES.map((lang, index) => {
        ...
        <button class="lang-option w-full flex items-center p-3 hover:bg-gray-100 ${roundedClass}" data-lang="${lang.code}" data-text="${lang.text}">
            <img src="https://flagcdn.com/${lang.flag}" alt="${lang.alt}" class="h-4 w-6 mr-2 object-contain">${lang.text}
        </button>`;
    }).join('');
...
window.getLanguageDropdownHTML = getLanguageDropdownHTML;